...
"$ENGINE_DIRECTORY/Engine/Build/BatchFiles/RunUAT.sh" BuildCookRun -project="$PROJECT" -build -$PLATFORM -clientconfig=$CLIENT_CONFIG -distribution -cook -maps=AllMaps -stage -package -archive -archivedirectory="$ARCHIVE_DIRECTORY"